...
export class DevelopmentSynthesizer implements Synthesizer {
  ...
  async *stream(input: SynthesisInput, signal: AbortSignal): AsyncIterable<string> {
    const chunks = [
      `Fleet investigated “${input.question}” from ${input.findings.length} distinct angles. `,
      input.findings.map((item) => item.finding).join('\n\n'),
      '\n\nThis answer was generated by the deterministic development synthesizer.',
    ]
